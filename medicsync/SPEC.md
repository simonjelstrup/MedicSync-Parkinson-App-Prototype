...
The user can also take a dose *earlier* than scheduled — for example, if they know they will be unable to confirm at the scheduled time, or if they have already taken the dose without using the app. The Home screen offers a subtle "Dose taken early" action on the next-dose card. Tapping it opens a bottom sheet where the user enters the actual time the dose was taken. The dose is logged with that time, the scheduled alarm is cancelled, and the "okay to eat" follow-up is scheduled accordingly. This is treated as a legitimate path, not an exception, but is visually understated to keep the standard NFC-based flow as the primary path.
...
### 3.6 Iconography rule

**No emojis anywhere in the app.** Every icon is a custom SVG that matches the app's visual language: line-art with consistent stroke width, terracotta or sage as the dominant colours, no decorative gradients. This rule is universal — UI icons, button glyphs, status indicators, content illustrations, and the OK-to-eat icon are all SVG. Emoji break the visual coherence of the warm, calm aesthetic and read as out-of-place.
...
| Schedule | Tab | Full routine overview, per-dose silence |
| History | Tab | Past doses by day |
| Settings | Tab | Configuration: doses, meal timing, profiles, language, account, pill box, relatives |
| Profile editor | Sub-screen of Settings | Edit Home or Outdoors alarm profile (sound, escalation, pattern) |
| Dose editor | Sub-screen of Settings | Add or edit a dose entry (label, timing, medications) |
| Pill box location | Sub-screen of Settings | View pill box status, configure leave-home alert |
...
| Bottom sheet: time picker | Modal | Used for setting meal times |
| Bottom sheet: dose taken early | Modal | Time entry for retroactive dose logging |
| Bottom sheet: language picker | Modal | Switch between English and Danish |
...
- Section: "Next dose"
  - Expanded card showing next dose:
    ...
    - **"Dose taken early"** button — subtle, secondary styling matching "Set exact time" (transparent background, soft amber border, secondary text colour). Opens a bottom sheet for time entry. Visually understated to avoid drawing focus from the main flow.
    - When expanded:
...
**The "Dose taken early" bottom sheet**

Triggered from the subtle button on the next-dose card. Layout:
- Title: "Dose taken early"
- Sub-text: "When did you take it?"
- A summary card showing the medications being logged
- A native time input, defaulted to the current time
- Cancel and Confirm buttons

When confirmed, the dose is logged with `takenAt` set to the entered time and `method: 'manual'`. The scheduled alarm for that dose is cancelled. The "okay to eat" follow-up is scheduled based on the entered time. The Home screen's next-dose card automatically updates to show the following dose.

**Behaviours:**
- Tapping the chevron collapses or expands the next-dose card.
- Tapping "Dose taken early" opens the bottom sheet described above.
...
- If the user takes a dose early via "Dose taken early", the next dose card automatically updates to show the following dose.
...
**Purpose:** Configure the user's routine, medications, alarm profiles, pill box, and relatives setup. All changes propagate in real time to every other screen.
...
1. **Your medication doses** — the user's complete dose schedule lives here. Each dose is editable.
   ...
   - Each entry is tappable, opening the dose editor (section 5.12) where the user can:
...
4. **Alarm profiles** — two rows: Home, Outdoors. Each tappable, opens the profile editor (section 5.9).
...
7. **Pill box** — single row "Pill box location". Tappable, opens the pill box location screen (section 5.13). Sub-text shows the current status, e.g. "At home · Last seen 2 min ago".

8. **Relatives** — toggle for "Notify if dose missed", row to view relatives view. Note: requires a signed-in account if the relative is to receive remote notifications. Without an account, the relatives view works as a local-only proof-of-concept.
...
- Sub-label: "Sound pattern"
- Four options describing how the sound plays:
  - **Constant** — plays continuously
  - **Intervals** — 5 sec on, 5 sec off, repeats
  - **Every 30 sec** — single chime every 30 seconds
  - **Every minute** — single chime every minute
- Save button

The sound pattern interacts with volume escalation: across all patterns, the loudness still ramps up over the chosen escalation duration. The pattern controls *how* the sound is delivered, the escalation controls *how loud* it gets over time.
...
**Purpose:** Add a new dose, or edit an existing one. Reached by tapping a dose card or "Add a dose" in Settings.

**Layout, top to bottom:**
- Back button "Settings"
- Eyebrow + title: "Edit dose" / [dose name]
- Field group: Dose label
  - Text input pre-filled with current label (e.g. "Lunch")
- Field group: Timing
  - Anchor builder: a sentence-style row that reads as a phrase, with three pill-style selectors:
    - Minutes (e.g. "30 min")
    - Relation (e.g. "before" / "after")
    - Meal (e.g. "lunch" / "breakfast" / "dinner")
  - The phrase reads as natural language: "Take [30 min] [before] [lunch]"
- Field group: Medications
  - List of medications, each row showing name, dosage, and a remove button (X icon)
  - "Add medication" button — opens a small form for new medication
- Save dose button (primary)
- Delete this dose button (subtle, soft red border) — only shown when editing an existing dose, not when adding new

**Behaviours:**
- All edits propagate in real time to Home, Schedule, and alarm scheduling on Save
- Deleting a dose requires confirmation (native `confirm()` is acceptable for v1; a custom dialog can be added later)
- "Add a dose" opens this editor with empty values and no Delete button

### 5.13 Pill box location

**Purpose:** Help the user find their pill box, and configure the alert that fires when they leave home without it.

**Layout, top to bottom:**
- Back button + title "Pill box location" + subtitle "Help you find it · keep it with you"
- Section: "Current status"
- Status card:
  - Header row: large icon (success or warning state), status title (e.g. "Pill box is at home" or "Pill box is X metres from home"), sub-text with last seen timestamp
  - Stylised map preview showing the pill box location with a coloured pin (green when at home, terracotta or red when away)
  - Action button: "Make pill box play a sound" (primary terracotta) — sends a ping signal to the pill box hardware to help locate it nearby
- Section: "Reminder settings"
- Toggle row: "Notify if I leave home without it" (default ON)
- Setting row: "Notify after [100 metres from home]" — tappable to adjust the radius. Possible values: 50m, 100m, 250m, 500m.

**Technical model:**
- The pill box contains a Bluetooth tracker with its own location capability (GPS or community-network-based)
- The phone holds a defined "home" location set during onboarding (or auto-detected from frequent location)
- A geofence is registered around the home location at the configured radius
- When the geofence is exited AND the pill box is still inside it, the alert fires
- The pill box's last-known location is shown on the map view
- "Make pill box play a sound" works only when the box is within Bluetooth range (~10m) of the phone

### 5.14 Lock screen notifications

**Purpose:** The app fires two distinct types of lock-screen notifications. Each renders as a rich notification card on the device's lock screen.

**Type 1 — Alarm notification (dose due)**
- App icon and "MedicSync" label
- Title: "Time for your dose"
- Body: medication names · meal context (e.g. "Levodopa 100mg · Amantadine 100mg / Before lunch · 12:00")
- NFC action panel showing phone-to-pill-box icon and "Tap phone to pill box" instruction
- Hint text below the card: "Open MedicSync to confirm without pill box"
- This is the same notification described in Phase 4 of the build plan

**Type 2 — Pill box left at home (warning)**
- App icon and "MedicSync" label
- Title: "Pill box left at home"
- Body: "You've moved more than 100 metres from home without your pill box. Did you mean to leave it?"
- No action panel (this is an informational notification)
- Tone is gentle and non-intrusive — a friendly check-in, not an alarm
- Visual difference from Type 1: warning icon in the notification icon position (using a shield outline or similar), but otherwise styled identically to maintain visual continuity
...
type AlarmProfile = {
  ...
  pattern: 'constant' | 'intervals' | 'every30sec' | 'everyMinute';
};

type PillboxState = {
  paired: boolean;
  lastKnownLocation: { lat: number; lng: number; timestamp: string } | null;
  lastSeenAtHome: string | null;       // ISO timestamp
  isCurrentlyAtHome: boolean;
};

type PillboxSettings = {
  leaveHomeAlertEnabled: boolean;       // default true
  alertRadiusMetres: 50 | 100 | 250 | 500;  // default 100
...
The pill box's last-known location and pairing state may sync to the account when signed in, so a user who replaces their phone can immediately reconnect without losing the pill box association.
...
### 7.2 Volume escalation and sound pattern
...
The **sound pattern** controls how the sound is delivered, independent of escalation:
- **Constant:** the chosen sound plays continuously
- **Intervals:** sound plays for 5 seconds, silent for 5 seconds, repeats
- **Every 30 seconds:** a single chime fires every 30 seconds, no continuous sound
- **Every minute:** a single chime fires every minute

Pattern and escalation combine: the loudness curve follows the escalation setting, and within that curve, the sound is delivered according to the pattern. So a "Slow + Every 30 seconds" alarm fires a single chime that gets louder each time over 5 minutes; a "Fast + Constant" alarm plays continuously and ramps to maximum in 1 minute.

When in silent mode (per-dose silence): no sound. Vibration pattern follows the same combination of escalation and pattern.
...
- All daily adjustments (shifts, custom times, per-dose silence) reset
...
### 7.7 Early dose intake ("Dose taken early")

The user may take a dose before its scheduled alarm time. They open the app, tap "Dose taken early" on the next-dose card, and enter the time they actually took the medication. The app:

1. Logs the dose with `takenAt` set to the entered time, `method: 'manual'`, `status: 'taken'`
2. Cancels the scheduled alarm for that dose so it does not fire
3. Schedules the "okay to eat" notification using the entered time as the reference (e.g. if 30 min before meal, the notification fires 30 minutes after the entered time)
4. Updates the Home screen's next-dose card to show the following dose

The entered time must not be in the future; if the user enters a future time, show a validation error and ask them to enter a past time.

### 7.8 Pill box left at home

The phone has a stored "home location" set during pill box pairing (or during onboarding). A geofence is registered around this location at the user's configured radius (default 100m).

When the phone exits the geofence, the app checks the pill box's last known location:
- If the pill box is also outside the geofence (i.e. the user has it with them), no alert
- If the pill box is still inside the geofence (i.e. left at home), fire the "Pill box left at home" notification

The notification is gentle and friendly — it is a check-in, not an alarm:
- Fires as a local push notification
- No sound (uses the device's default notification sound at low priority); vibrates briefly
- Title: "Pill box left at home"
- Body: "You've moved more than X metres from home without your pill box. Did you mean to leave it?"
- No action panel — purely informational
- Fires only once per geofence exit; does not repeat

If the user disables the alert in Settings, no notification fires. If they re-enter the geofence and exit again, a new notification can fire.

**Edge cases:**
- The pill box's location is updated on a best-effort basis (Bluetooth tracker networks have variable reliability). If the pill box's location is unknown for more than 6 hours, the alert does not fire — the app cannot reliably determine whether it was left behind.
- The phone's location must be permitted in foreground and background for this feature to work. If permission is denied, the feature is disabled and the toggle in Settings is greyed out with a tappable hint to grant permission.
...
- `expo-location` — for the home geofence and pill box "left at home" detection (Phase 5+)
- `react-i18next` — for English/Danish localisation (Phase 1)
...
For pill box Bluetooth tracking (later phase): `react-native-ble-plx` or similar. Also requires development builds. The pill box's exact hardware/protocol is TBD; the app should treat it through an abstraction (`pillboxService.ts`) so the underlying implementation can change without affecting the rest of the app.
...
│   └── PillboxStatusCard.tsx
├── lib/
│   ├── theme.ts                  # colour and type tokens
│   ├── i18n.ts                   # i18next setup, language switcher
│   ├── confirmationService.ts
│   ├── alarmScheduler.ts
│   ├── notifications.ts
│   ├── storage.ts
│   ├── pillboxService.ts         # Bluetooth + location abstraction
│   └── geofence.ts               # home geofence registration
├── stores/
...
Deliverable: alarms actually fire on schedule. Manual confirmation works end to end. "OK to eat" follow-up fires. Lock screen cards display correctly.
...
- Build **lock-screen notification type 1: alarm card** matching the design in `medicsync.html` (use the "Lock: alarm" demo button to see the target). The card must show:
  - App icon and "MedicSync" label
...
- Play sound with volume escalation **and the chosen sound pattern** (constant, intervals, every 30 sec, every minute) — see section 7.2
- Handle silent mode (per-dose silence)
- Wire up the manual confirmation flow → calls `confirmDose(id, 'manual')`
- Wire up the "Dose taken early" bottom sheet on Home → on confirm, calls `confirmDose(id, 'manual')` with the entered time, cancels the scheduled alarm for that dose
...
- Build the **Dose editor** screen (section 5.12), wired up to the dose store with full add/edit/delete behaviour
- Build the **Pill box location** screen as a static UI (section 5.13) — the toggle and radius selector work and persist; actual location detection is in Phase 5.5
- Build the **Dose taken early** bottom sheet (section 5.1) and wire it up
- Empty states (no doses today, no history yet)
- Error states (notification permission denied, time picker validation, future-time-entered validation)
...
### Phase 5.5 — Pill box location detection

Deliverable: the "left at home" notification fires correctly based on real location data.

- Request location permissions (foreground and background, with clear in-context explanation)
- Implement home geofence registration via `expo-location` based on the user's stored home location (set during onboarding or pill box pairing)
- Build **lock-screen notification type 2: pill box left at home** matching the design in `medicsync.html` (use the "Lock: left at home" demo button to see the target). The card shows:
  - App icon and "MedicSync" label, warning icon variant
  - Title: "Pill box left at home"
  - Body: "You've moved more than X metres from home without your pill box. Did you mean to leave it?"
  - No action panel — informational only, gentle tone
- Implement the geofence-exit handler that checks the pill box's last-known location and fires the notification only when the box was left behind
- Wire up the toggle and radius selector on the Pill box location screen to actually control geofence registration
- For v1 without real pill box hardware, implement a stub `pillboxService` that pretends the box is at home — this lets the user-facing flow be tested before hardware exists

End-of-phase test: with the toggle ON, walk more than 100m from home with the phone (and the simulated pill box "at home"). The "Pill box left at home" notification fires once on lock screen.
...
- Real pill box pairing flow over Bluetooth
- Replace stub `pillboxService` with real implementation
- Validation that the tapped NFC tag matches the user's paired box (not just any tag)
- Pill box "play a sound" feature wired up to actual hardware
- Calibration of geofence detection with real pill box location data
...
User testing for v1 is conducted with **Danish-speaking participants**. The Danish translation is therefore not optional or partial — it must be complete and feel native by the time of testing. Every user-facing surface should be testable end-to-end in Danish without any English text leaking through.

### 11.1 What gets translated

Every user-facing string: screen titles, labels, body text, button text, notification titles and bodies, error messages, the "okay to eat" notification body, status badges, alarm contract text, onboarding flow, dose editor labels, pill box screen, lock-screen notifications, and bottom sheet contents.

### 11.2 What does not get translated

- Medication names (these are proper nouns set by the user themselves)
- User-entered text (dose labels, custom names)
- Time formats follow the user's system locale
- The app brand name "MedicSync"

### 11.3 Implementation

Use `react-i18next` with locale files in JSON. Structure:
...
Every string in the codebase is referenced by a key (e.g. `t('home.greeting.morning')`). The store holds the active language; switching it triggers an immediate re-render of every screen via the reactive state described in section 2.7. No app reload is required.

For parameterised strings (e.g. "Reminder at {remind} · lunch at {lunch}"), use placeholder substitution rather than concatenating translated fragments. Concatenation breaks word order in non-English languages.

### 11.4 Tone in Danish

Danish translations match the warm, calm, non-clinical tone of the English copy. Use the informal "du" form throughout — Denmark uses this universally in app interfaces and consumer products. Avoid medical jargon. Translate meaning, not words — for example, "Time for your dose" in Danish reads as "Tid til din medicin" (more natural than a literal translation of "dose"); "Dose taken early" reads as "Dosis taget tidligt".

A native Danish speaker reviews all translations before any user testing session. Machine translation is acceptable as a first pass but not for the final product or any version exposed to participants.
...
- The user can log a dose retroactively via "Dose taken early" with a custom time
- The "okay to eat" notification fires correctly based on user setting
- The "Pill box left at home" notification fires when the user crosses the geofence without the box (Phase 5.5+)
- Daily adjustments (shifts, per-dose silence) work and reset correctly at midnight
- History accurately reflects taken via pill box, taken manually, missed, and upcoming doses, distinctly
- All settings persist across app restarts and sync to the user's account when signed in
- Edits in Settings (medications, meal times, language, profiles, pill box settings) propagate immediately to every screen — no stale views
- Alarm profiles support sound, volume escalation speed, and sound pattern (constant / intervals / every 30 sec / every minute)
- The full app is available in English and Danish, with language switchable at any time. No emoji is used as content; all icons are SVG matching the app's style.
...
- The codebase is structured so that real pill box hardware can replace the stub `pillboxService` in a single isolated module
...
## 15. As-built development history

This section records how the app was actually built, which differs from the phased plan in section 9. It serves as a truthful record for anyone picking up the project.

---

### 15.1 What was planned vs. what happened

The original plan had eight phases, with NFC deferred to Phase 6 and the switch to development builds deferred to the same point. In practice the project stopped after Phase 4 — but Phase 4 was significantly extended. NFC integration (originally Phase 6) was folded into Phase 4 once it became clear that development builds were already required for audio. Phases 5 through 8 were not built.

| Planned phase | Status |
|---|---|
| Phase 1 — Design system and primitives | ✅ Built as planned |
| Phase 2 — Static screens with mock data | ✅ Built as planned |
| Phase 3 — State management and persistence | ✅ Built as planned |
| Phase 4 — Alarms and notifications | ✅ Built and extended (see 15.5) |
| Phase 5 — Onboarding, polish, and edge cases | ❌ Not built |
| Phase 5.5 — Pill box location detection | ❌ Not built |
| Phase 6 — NFC integration | ✅ Folded into Phase 4 |
| Phase 7 — Accounts and backend | ❌ Not built |
| Phase 8 — Hardware integration | ❌ Not built |

---

### 15.2 Phase 1 — Design system and primitives

Built on Expo SDK 54, running in Expo Go. No navigation yet — a single Showcase screen displayed all primitive components.

**Delivered:**
- `lib/theme.ts` — 30+ colour tokens, full typography scale, spacing, radii, shadows
- `lib/i18n.ts` — react-i18next configured with English and Danish locale files
- `components/primitives/` — Button (primary/secondary/tertiary/destructive), Card, Toggle (Reanimated), RadioGroup, Badge (6 statuses), IconButton, BottomSheet
- Manrope font loaded at weights 400/500/600/700

**Key technical issue resolved:** `react-native-reanimated ~4.1.1` installs `react-native-worklets@0.8.x` as a transitive dep, but Expo Go SDK 54 ships `worklets@0.5.1` natively. Fixed with a `package.json` override pinning worklets to `0.5.1`.

---

### 15.3 Phase 2 — Static screens with mock data

Expo Router v6 added. All screens built with hard-coded mock data. No persistence.

**Delivered:**
- Full tab navigation (Today / Schedule / History / Settings)
- All overlay screens (alarm, manual-confirm, confirmed, ok-to-eat)
- Sub-screens (dose-editor, profile-editor, pillbox-location, relatives)
- All bottom sheets (time picker, taken early, language picker)
- `SilenceContext` — React Context sharing per-dose silence state across tabs in real time
- `components/svg/Icons.tsx` — ~25 custom SVG icons; no emoji anywhere
- Full English and Danish locale files for every user-facing string

**Key technical issue resolved:** `react-native-screens` resolved to `4.24.0` via transitive nav deps, but Expo Go's native binary was compiled against `~4.16.0`. Fixed by pinning `react-native-screens` as an explicit direct dep at `~4.16.0`.

---

### 15.4 Phase 3 — State management and persistence

All mock data replaced with Zustand stores persisted to AsyncStorage.

**Delivered:**
- `stores/scheduleStore.ts` — user's dose schedule (`StoredDose[]`, add/update/delete)
- `stores/settingsStore.ts` — meal times, alarm profiles, language, relatives settings
- `stores/todayStore.ts` — per-day meal adjustments, active alarm profile, day rollover
- `stores/historyStore.ts` — all confirmed and missed dose logs
- `lib/scheduleUtils.ts` — `useComputedDoses()`, `useTodayDoses()`, `useHistoryDays()`, and the timing computation functions
- Data model extended with `timingMode: 'meal' | 'fixed'` and `fixedTime` on doses
- Day rollover: `checkRollover()` called on app foreground; clears daily adjustments and silence states at midnight
- Dose editor rebuilt to use inline pickers (bottom sheet pickers were intercepting Save button touches)

---

### 15.5 Phase 4 (extended) — Alarms, notifications, audio, and NFC

This phase was substantially larger than planned. It includes the original Phase 4 scope plus the full NFC integration originally planned for Phase 6. This was the final phase of development.

**Core alarm system delivered:**
- `lib/notifications.ts` — permission request, Android channels, notification categories
- `lib/alarmScheduler.ts` — schedule/cancel dose alarms, schedule ok-to-eat, mark missed doses
- `lib/confirmationService.ts` — `confirmDose(doseId, method, takenAt?)` — single entry point for all confirmation paths
- `lib/soundManager.ts` — `useAlarmSound(profile, silenced, startTimeRef)` hook with escalation (volume ramps from 10% to 100% over the profile's escalation window) and pattern playback (constant, intervals, every-30-sec, every-minute)
- Lock-screen notification category `dose-alarm` with "Confirm without pill box" action button; tapping it from the lock screen navigates directly to `/manual-confirm`
- Screen kept awake during active alarm via `expo-keep-awake`
- Alarm sound plays even when iOS ringer is off (`playsInSilentModeIOS: true`)
- Switch from Expo Go to **EAS development builds** (required for expo-av and native modules)
- `eas.json` with development, device, preview, and production profiles

**NFC integration (originally Phase 6):**
- `lib/nfcService.ts` — `isNfcSupported()`, `scanForTag(alertMessage)`, `cancelScan()`; supports `[NfcTech.Ndef, NfcTech.NfcA, NfcTech.NfcB, NfcTech.NfcV, NfcTech.IsoDep]` for broad tag compatibility
- Alarm screen auto-starts NFC scan 1.2 seconds after mount; the iOS system NFC sheet appears automatically
- On successful NFC read: `confirmDose(doseId, 'nfc')` called; alarm stops; navigates to confirmed screen
- NFC entitlement added (`com.apple.developer.nfc.readersession.formats`); requires a paid Apple Developer account
- `plugins/withNfcEntitlement.js` — Expo config plugin that persists the entitlement through prebuilds
- `NFCReaderUsageDescription` added to `ios/MedicSync/Info.plist`

**Build infrastructure issues resolved during Phase 4:**
- **Xcode 26.5 / Swift 6**: defaulted to strict concurrency checking, breaking several pods. Fixed by adding a `post_install` hook to `ios/Podfile` that sets `SWIFT_STRICT_CONCURRENCY = minimal` and `SWIFT_VERSION = 5.0` for all targets. **This hook must be re-added after every `npx expo prebuild --clean`.**
- **expo-image Swift 6 error**: `static let center = Self()` not Sendable. Fixed via `patch-package` (`patches/expo-image+55.0.10.patch`) adding `nonisolated(unsafe)`.
- **expo-dev-menu Swift 6 error**: `reloadAppAsync()` missing on AppContext. Fixed via `patch-package` (`patches/expo-dev-menu+55.0.29.patch`) replacing it with `self.bridge?.reload()`.
- **SDK 54 → 55 upgrade**: Required when incompatible header versions caused build failures after Xcode 26.5 upgrade. Performed with `--legacy-peer-deps` throughout; followed by `npm run prebuild:ios`.

---

### 15.6 Post-Phase 4 improvements

After the core four phases were complete, a series of smaller improvements were made in subsequent sessions.

**Pillbox location screen:**
- Removed the static SVG map illustration from the status card
- Added "Find AirTag in Find My" button that opens the Find My app via `findmy://` deep link (falls back to App Store if not installed)

**Settings screen:**
- "Notify if dose missed" now shows four time-option chips (5 min / 15 min / 30 min / 1 hour) when the toggle is on, persisted to `settingsStore`
- Section order changed: Pill Box moved above Account
- Relatives view row uses `PersonIcon` instead of `FamilyIcon`

**Home screen:**
- Empty state now distinguishes "no doses configured" (directs user to Settings) from "all doses taken for today"
- Meal shift label and "shifted to" feedback are now meal-specific (e.g. "Shift dinner time" instead of always "Shift lunch time")

**Schedule screen:**
- Dose labels ("Morning", "Lunch", etc.) now translated via i18n — previously rendered the raw English string regardless of language

**History screen:**
- "Clear history" button added to the header; triggers a native `Alert` confirmation before wiping logs
- Button hidden when history is empty

**DoseSilenceBell component:**
- Restyled as a visible pill-shaped button with a border and background colour, replacing the plain bare icon, to improve tap affordance

**Dose ordering:**
- `useComputedDoses()` now sorts by `scheduledTime`, ensuring Schedule tab and Settings tab always reflect time order regardless of creation order

**i18n fixes:**
- "1h" chip in the notify-missed selector uses `doseEditor.oneHour` ("1 time" in Danish) instead of a hardcoded string

**App icon:**
- Updated to `assets/newicon.png` (1024×1024 PNG) across all references in `app.json` (main icon, Android adaptive icon foreground, expo-notifications icon); regenerated via `npx expo prebuild --platform ios --clean`
